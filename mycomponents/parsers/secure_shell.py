...
class SSHDConfig(Parser):
    ...
    """namedtuple: Represents name value pair as namedtuple with case."""

    def parse_content(self, content):
        self.lines = []
        for line in get_active_lines(content):
            keyword, value = line.split(None, 1)
            self.lines.append(self.KeyValue(keyword.strip(), value.strip(), keyword.lower().strip()))
        self.keywords = set([kv.kw_lower for kv in self.lines])
